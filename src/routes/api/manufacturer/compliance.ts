...
export const Route = createAPIFileRoute("/api/manufacturer/compliance")({
    GET: async ({ request }: { request: Request }) => {
        try {
            const payload = await authorizeRequest(request, ["MANUFACTURER"]);
            const metrics = await ManufacturerDocumentService.getComplianceMetrics(
                payload.userId
            );
            return Response.json(ApiResponse.success(metrics, "Compliance metrics loaded."));
        } catch (error: any) {
