...
export const Route = createAPIFileRoute("/api/admin/documents/$id/reject")({
    PUT: async ({ request, params }: { request: Request; params: { id: string } }) => {
        try {
            ...
            const document = await ManufacturerDocumentService.adminReject(
                payload.userId,
                params.id,
                body
            );
            return Response.json(ApiResponse.success(document, "Document rejected."));
        } catch (error: any) {
